...
type Item = { id: string; slug: string; label: string; status?: "DRAFT" | "PUBLISHED" | "ARCHIVED"; version?: string };
...
export function TaxonomyManager({ title, items, save, archive }: { title: string; items: Item[]; save: Action; archive?: Action }) {
  ...
  return <Card title={title}><Form form={form} layout="inline" initialValues={{ status: "DRAFT" }} onFinish={submit}>
    ...
    <Form.Item name="slug" rules={[{ required: true }]}><Input placeholder="slug" /></Form.Item><Form.Item name="title" rules={[{ required: true }]}><Input placeholder="English name" /></Form.Item>
    ...
    <Button key="edit" size="small" onClick={() => form.setFieldsValue({ id: item.id, version: item.version, slug: item.slug, title: item.label, status: item.status ?? "DRAFT" })}>Edit</Button>,
