...
import { Button, Card, Modal, Progress, Space, Typography, Upload } from "antd";
...
export function uploadObjectWithProgress(
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    xhr.onload = () => {
      ...
      reject(new Error(`无法上传媒体文件（${xhr.status}）`));
    };
    xhr.onerror = () => reject(new Error("网络中断，媒体文件上传失败"));
    xhr.onabort = () => reject(new Error("媒体文件上传已取消"));
    xhr.send(file);
  });
}
...
export function MediaPicker({
  ...
  const uploading = uploadProgress?.status === "active";
  ...
  const customRequest: NonNullable<UploadProps["customRequest"]> = async (request) => {
    let lastPercent = 0;
    const updateProgress = (fileName: string, phase: string, percent: number, status: UploadProgressState["status"] = "active") => {
      const normalizedPercent = Math.min(100, Math.max(0, Math.round(percent)));
      lastPercent = normalizedPercent;
      setUploadProgress({ fileName, phase, percent: normalizedPercent, status });
      request.onProgress?.({ percent: normalizedPercent });
    };
    try {
      ...
      updateProgress(file.name, "正在创建安全上传地址", 5);
      ...
      updateProgress(file.name, "正在上传到媒体库", 10);
      await uploadObjectWithProgress(signed, file, (percent) => updateProgress(file.name, "正在上传到媒体库", 10 + percent * 0.7));
      updateProgress(file.name, "正在确认上传结果", 88);
      ...
      updateProgress(file.name, "上传完成", 100, "success");
      request.onSuccess?.(asset);
    } catch (error) {
      const failure = error instanceof Error ? error : new Error("媒体上传失败");
      setUploadNotice(failure.message);
      setUploadProgress((current) => ({ fileName: current?.fileName ?? "媒体文件", phase: failure.message, percent: lastPercent, status: "exception" }));
  ...
  return <Card size="small" title="产品媒体">
    <Space orientation="vertical" style={{ width: "100%" }}>
      ...
      <div className="admin-media-actions"><Upload {...upload} customRequest={upload?.customRequest ?? customRequest} accept=".jpg,.jpeg,.png,.webp,.avif,image/jpeg,image/png,image/webp,image/avif" showUploadList={false} disabled={uploading || upload?.disabled}><Button loading={uploading}>上传照片</Button></Upload>{showCatalogue ? <Button onClick={() => setLibraryOpen(true)}>从媒体库选择</Button> : null}<Button onClick={clear} disabled={!selectedIds.length}>清除选择</Button></div>
      {uploadProgress ? <div className="admin-upload-progress" role="status" aria-live="polite">
        <div className="admin-upload-progress__header"><Typography.Text strong>{uploadProgress.fileName}</Typography.Text><Typography.Text type={uploadProgress.status === "exception" ? "danger" : "secondary"}>{uploadProgress.phase}</Typography.Text></div>
