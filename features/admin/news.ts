...
const articleInputSchema = z.object({ id: z.string().min(1).optional(), categoryId: z.string().min(1), slug: z.string().trim().regex(/^[a-z0-9]+(?:-[a-z0-9]+)*$/), coverMediaId: z.string().min(1).nullable().optional(), tagIds: z.array(z.string().min(1)).max(30), scheduledAt: z.coerce.date().nullable().optional(), status: z.enum(["DRAFT", "PUBLISHED", "ARCHIVED"]).default("DRAFT"), translations: translationsSchema });
export type NewsAdminInput = z.infer<typeof articleInputSchema>;
export type NewsAdminRepository = { saveArticle(input: NewsAdminInput & { actorId: string }): Promise<{ id: string; slug: string }>; countArticlesInCategory?(categoryId: string): Promise<number>; archiveCategory?(categoryId: string, actorId: string): Promise<void>; countArticlesWithTag?(tagId: string): Promise<number>; archiveTag?(tagId: string, actorId: string): Promise<void> };
...
export function createNewsAdminService(dependencies: { repository: NewsAdminRepository; invalidate(type: "article", slug: string): void; now?: () => Date }) {
  return {
    ...
    async archiveCategory(input: { actor: AdminEditorActor | null; categoryId: string }) {
      ...
      if (await dependencies.repository.countArticlesInCategory(input.categoryId)) throw new EditorValidationError("This category is referenced by articles and cannot be archived"); await dependencies.repository.archiveCategory(input.categoryId, input.actor.id);
    },
    async archiveTag(input: { actor: AdminEditorActor | null; tagId: string }) {
      ...
      if (await dependencies.repository.countArticlesWithTag(input.tagId)) throw new EditorValidationError("This tag is referenced by articles and cannot be archived"); await dependencies.repository.archiveTag(input.tagId, input.actor.id);
    },
  };
}
