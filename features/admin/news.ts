...
export function createNewsAdminService(dependencies: { repository: NewsAdminRepository; invalidate(type: "article", slug: string): void; now?: () => Date }) {
  return {
    ...
    async archiveCategory(input: { actor: AdminEditorActor | null; categoryId: string; version: string }) {
      ...
      const data = z.object({ categoryId: z.string().min(1), version: z.string().datetime() }).parse(input); if (await dependencies.repository.countArticlesInCategory(data.categoryId)) throw new EditorValidationError("This category is referenced by articles and cannot be archived"); await dependencies.repository.archiveCategory(data.categoryId, input.actor.id, data.version);
    },
    async archiveTag(input: { actor: AdminEditorActor | null; tagId: string; version: string }) {
      ...
      const data = z.object({ tagId: z.string().min(1), version: z.string().datetime() }).parse(input); if (await dependencies.repository.countArticlesWithTag(data.tagId)) throw new EditorValidationError("This tag is referenced by articles and cannot be archived"); await dependencies.repository.archiveTag(data.tagId, input.actor.id, data.version);
    },
  };
}
