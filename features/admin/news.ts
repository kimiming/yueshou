...
const translationsSchema = z.array(z.object({ locale: z.enum(contentLocales), title: z.string().trim().min(1).max(160), body: z.string().trim().min(1), excerpt: z.string().trim().max(500).optional() })).min(1).max(contentLocales.length);
...
export type NewsAdminRepository = { saveArticle(input: NewsAdminInput & { actorId: string }): Promise<{ id: string; slug: string }>; countArticlesInCategory?(categoryId: string): Promise<number>; archiveCategory?(categoryId: string, actorId: string): Promise<void> };
