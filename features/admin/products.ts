...
export function createProductAdminService(dependencies: { repository: ProductAdminRepository; invalidate(type: "product", slug: string): void; now?: () => Date }) {
  return {
    ...
    async archiveCategory(input: { actor: AdminEditorActor | null; categoryId: string; version: string }) {
      ...
      const data = z.object({ categoryId: z.string().min(1), version: z.string().datetime() }).parse(input); await dependencies.repository.archiveCategory(data.categoryId, input.actor.id, data.version);
    },
  };
}
