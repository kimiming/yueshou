...
const translationsSchema = z.array(z.object({ locale: z.enum(contentLocales), title: z.string().trim().min(1).max(160), body: z.string().trim().min(1) })).min(1).max(contentLocales.length);
