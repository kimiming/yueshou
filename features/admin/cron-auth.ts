...
export function verifyVercelCronRequest(input: { secret?: string; authorization?: string | null }) {
  if (!input.secret || input.secret.length < 32 || !input.authorization) return false;
  const expected = Buffer.from(`Bearer ${input.secret}`); const supplied = Buffer.from(input.authorization);
