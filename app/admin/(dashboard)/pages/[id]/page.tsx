...
export default async function PageEditorPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!page) notFound();
  const savePayload = { id: page.id, slug: page.slug, version: page.updatedAt.toISOString(), status: page.status, translations: page.translations.map((item) => ({ locale: (item.locale === "zh_CN" ? "zh-CN" : item.locale) as "en" | "zh-CN" | "de" | "fr" | "es", title: item.title, body: item.body, seoTitle: item.seoTitle ?? undefined, seoDescription: item.seoDescription ?? undefined })) };
