...
export async function savePageAction(input: unknown) { const actor = await requireUser(); const body = readPayload(input) as { id?: string }; const old = body.id ? await prisma.page.findUnique({ where: { id: body.id }, select: { slug: true } }) : null; const result = await service().savePage({ ...body, actor }); await invalidatePublishedPage(result.id, old?.slug); refresh(result.id); return result; }
export async function savePageAndSetStatusAction(input: unknown) { const actor = await requireUser(); const body = readPayload(input) as { id: string }; const old = await prisma.page.findUnique({ where: { id: body.id }, select: { slug: true, status: true } }); const result = await service().savePageAndSetStatus({ ...body, actor }); await invalidatePublishedPage(result.id, old?.status === "PUBLISHED" ? old.slug : undefined); refresh(result.id); return result; }
export async function approveLegalPageAction(input: unknown) { const actor = await requireUser(); const body = readPayload(input) as { pageId: string; version: string; contentRevision: number }; const result = await service().approveLegalPage({ ...body, actor }); refresh(result.id); return result; }
export async function savePageSectionAction(input: unknown) { const actor = await requireUser(); const body = readPayload(input) as { pageId: string }; const old = await prisma.page.findUnique({ where: { id: body.pageId }, select: { slug: true, status: true } }); await service().savePageSection({ ...body, actor }); await invalidatePublishedPage(body.pageId, old?.status === "PUBLISHED" ? old.slug : undefined); refresh(body.pageId); }
export async function reorderPageSectionsAction(input: unknown) { const actor = await requireUser(); const body = readPayload(input) as { pageId: string; orderedIds: string[] }; const old = await prisma.page.findUnique({ where: { id: body.pageId }, select: { slug: true, status: true } }); await service().reorderPageSections({ ...body, actor }); await invalidatePublishedPage(body.pageId, old?.status === "PUBLISHED" ? old.slug : undefined); refresh(body.pageId); }
