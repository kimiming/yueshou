...
describe("parseEnv", () => {
  ...
  it("rejects direct proxy mode in production while supporting trusted deployments", () => {
    const base = {
      NODE_ENV: "production", DATABASE_URL: "postgresql://db.test/app", AUTH_SECRET: "12345678901234567890123456789012",
      INQUIRY_HASH_SECRET: "12345678901234567890123456789012",
      STORAGE_ENDPOINT: "https://objects.test", STORAGE_REGION: "auto", STORAGE_BUCKET: "private", STORAGE_ACCESS_KEY_ID: "key", STORAGE_SECRET_ACCESS_KEY: "secret", NEXT_PUBLIC_SITE_URL: "https://site.test",
    };

    expect(() => parseEnv({ ...base, INQUIRY_PROXY_MODE: "direct" })).toThrow("INQUIRY_PROXY_MODE");
    expect(parseEnv({ ...base, INQUIRY_PROXY_MODE: "vercel" }).INQUIRY_PROXY_MODE).toBe("vercel");
    expect(parseEnv({ ...base, INQUIRY_PROXY_MODE: "nginx" }).INQUIRY_PROXY_MODE).toBe("nginx");
    expect(parseEnv({ ...base, NODE_ENV: "development", INQUIRY_PROXY_MODE: "direct" }).INQUIRY_PROXY_MODE).toBe("direct");
  });
});
