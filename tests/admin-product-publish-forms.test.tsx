...
it("shows a success dialog after creating and publishing a product", async () => {
  ...
  const editor = screen.getByRole("textbox", { name: "产品内容" });
  fireEvent.input(editor, { target: { innerHTML: "<p><strong>Research</strong> product</p>" } });
  ...
  await waitFor(() => expect(save).toHaveBeenCalledWith(expect.objectContaining({
    ...
    slug: "test-peptide",
    status: "PUBLISHED",
    translations: [expect.objectContaining({ locale: "en", body: "<p><strong>Research</strong> product</p>" })],
  })));
...
it("submits rich product content when editing and publishing", async () => {
  vi.spyOn(globalThis, "fetch").mockResolvedValue(new Response("[]", { status: 200 }));
  vi.spyOn(Modal, "success").mockImplementation(() => ({ destroy: vi.fn(), update: vi.fn() }));
  const save = vi.fn(async () => ({ id: "product-1", status: "PUBLISHED", version: "2026-08-13T00:00:01.000Z" }));
  render(<ExistingContentForm kind="product" initial={{ id: "product-1", version: "2026-08-13T00:00:00.000Z", categoryId: "category-1", slug: "test-peptide", status: "PUBLISHED", mediaIds: [], translations: [{ locale: "en", title: "Test peptide", body: "<p>Research product</p>" }] }} categories={[{ id: "category-1", label: "Research" }]} save={save} />);

  const editor = screen.getByRole("textbox", { name: "产品内容" });
  fireEvent.input(editor, { target: { innerHTML: "<h2>Application</h2><p><span style=\"background-color: #fff3a3\"><strong>Highlighted</strong></span></p><table><tbody><tr><td>Purity</td><td>98%</td></tr></tbody></table>" } });
  fireEvent.click(screen.getByRole("button", { name: "保存并立即发布" }));

  await waitFor(() => expect(save).toHaveBeenCalledWith(expect.objectContaining({
    translations: [expect.objectContaining({
      locale: "en",
      body: expect.stringContaining("<table>"),
    })],
  })));
});
